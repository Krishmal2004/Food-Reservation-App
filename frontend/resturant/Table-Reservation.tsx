...
import { StyleSheet, View, Text, TouchableOpacity, ScrollView, Modal, KeyboardAvoidingView, TextInput, Platform, Alert, ActivityIndicator } from 'react-native';
...
const TableReservation: React.FC<TableReservationProps> = ({ loggedInRestaurantId }) => {
  ...
  const handleOpenEditModal = (table: any) => {
    ...
    setTableState({ 
      ...
      time: table.time, 
      note: table.note || '',
      price: table.price ? table.price.toString() : '' // Added price
    });
    setModalVisible(true);
  };

  const handleSaveTable = async () => {
    // Added price to validation
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <View style={styles.modalContent}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} style={{maxHeight: 500}}>
                ...
                <Text style={styles.inputLabel}>Available Date (Calendar)</Text>
                <View style={styles.datePickerPseudo}>
                  <Text style={styles.pickerEmoji}>📅</Text>
                  <TextInput style={styles.pickerInput} placeholder="YYYY-MM-DD" value={tableState.date} onChangeText={t => setTableState({...tableState, date: t})} />
                </View>

                <Text style={styles.inputLabel}>Available Time (Clock)</Text>
                <View style={styles.datePickerPseudo}>
                  <Text style={styles.pickerEmoji}>⏰</Text>
                  <TextInput style={styles.pickerInput} placeholder="e.g. 19:00 or 7:00 PM" value={tableState.time} onChangeText={t => setTableState({...tableState, time: t})} />
                </View>
...
const styles = StyleSheet.create({
  ...
  datePickerPseudo: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F5F5F5', borderRadius: 8, borderWidth: 1, borderColor: '#E0E0E0', paddingHorizontal: 12 },
  pickerEmoji: { fontSize: 18, marginRight: 8 },
  pickerInput: { flex: 1, paddingVertical: 12, fontSize: 15, color: '#333' },
